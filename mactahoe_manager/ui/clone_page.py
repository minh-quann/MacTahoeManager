...
class ClonePage:
    def __init__(self, app_window, on_success_callback):
        ...
        status.set_description(
            f"Cần tải mã nguồn của MacTahoe Theme từ GitHub về máy trước khi cài đặt.\n"
            f"Thư mục lưu trữ: {cfg.DEFAULT_REPO_DIR}"
        )
        ...
        def clone_thread():
            try:
                ...
                if not os.path.exists(os.path.join(repo_dir, ".git")):
                    subprocess.run(["git", "clone", cfg.REPO_URL, repo_dir], check=True)
                else:
